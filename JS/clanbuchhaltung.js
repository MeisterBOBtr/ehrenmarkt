...
// Compatibility for the older render functions in this file.
// Always points to the currently loaded order-worker array.
let workers = [];

// Workers currently being edited in the order settlement modal.
let currentWorkers = [];
...
function updateBookingParties(){
    const type=(getValue('bookingType') || 'Einzahlung').trim().toLowerCase();
    const user=currentEmployee?.name || currentUser?.email || '—';
    if(type==='auszahlung'){
        setValue('bookingFrom','Clan-Kasse');
        setValue('bookingTo',user);
    }else{
        setValue('bookingFrom',user);
        setValue('bookingTo','Clan-Kasse');
    }
}
...
function v1IsMember(){ return v1Rank()==='mitglied' || v1Rank()==='mitarbeiter'; }
...
function fillV1EmployeeSelects(){
  const opts=employees.filter(e=>e.user_id).map(e=>'<option value="'+escapeHtml(e.user_id)+'">'+escapeHtml(e.name||e.username||e.minecraft_name||e.user_id)+'</option>').join('');
  ...
  const loan=document.getElementById('loanMember'); if(loan) loan.innerHTML='<option value="">Mitglied auswählen</option>'+employees.filter(e=>['mitglied','mitarbeiter'].includes(String(e.rang||'').toLowerCase())).map(e=>'<option value="'+escapeHtml(e.user_id)+'">'+escapeHtml(e.name||e.username||e.minecraft_name)+'</option>').join('');
  syncWorkerEmployeeSelect();
}

function syncWorkerEmployeeSelect(){
  const target=document.getElementById('workerName');
  if(!target) return;
  const current=target.value || '';
  target.innerHTML='<option value="">Mitarbeiter auswählen</option>' + employees.map(e=>{
    const name=e.name || e.username || e.minecraft_name || '';
    if(!name) return '';
...
async function saveIncomeEntry(){
  ...
  const amount=numberValue(document.getElementById('incomeAmount')?.value); const purpose=(document.getElementById('incomePurpose')?.value||'').trim(); if(amount<=0||!purpose){v1Status('incomeStatus','Betrag und Zweck sind erforderlich.',false);return;} if(!v1Confirm('Einnahme über '+money(amount)+' speichern?'))return;
...
function isMitarbeiter(){ const r=getCurrentRank().toLowerCase(); return r==='mitglied' || r==='mitarbeiter'; }
...
function openWorkerModal(){
  ...
  const salary=document.getElementById('workerSalary');
  const note=document.getElementById('workerNote');
  if(name) name.value='';
...
function addWorkerRow(){ openWorkerModal(); }
function saveWorker(){
  if(!v1CanManage()) return;
  ...
  if(!name){ v1Status('loanStatus','Bitte einen Mitarbeiter auswählen.',false); return; }
  if(salary<0){ v1Status('loanStatus','Das Gehalt darf nicht negativ sein.',false); return; }
...
window.openWorkerModal=openWorkerModal; window.closeWorkerModal=closeWorkerModal; window.syncWorkerEmployeeSelect=syncWorkerEmployeeSelect; window.addWorkerRow=addWorkerRow; window.saveWorker=saveWorker; window.openCashCheck=openCashCheck; window.closeCashCheck=closeCashCheck; window.closeLogDetail=closeLogDetail; window.saveSavingsTransaction=saveSavingsTransaction;
