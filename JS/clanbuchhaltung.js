...
async function loadSavingsGoal(){
    ...
    if(details && typeof details === "object"){
        savingsGoalValue = numberValue(details.goal);
    }
}
    ...
    try{
        ...
        */
        ...
        await notifyBookkeeping({
            route: type === "Einzahlung" ? "einzahlung" : "auszahlung",
...
async function saveSavingsEntry(){
    ...
    await notifyBookkeeping({
        ...
        type: "Sparkonto " + type,
        amount,
        from: type === "Einzahlung" ? getCurrentUserName() : "Sparkonto",
        to: type === "Einzahlung" ? "Sparkonto" : getCurrentUserName(),
        purpose,
        bookingNumber,
...
async function saveSavingsGoal(){
    ...
    let goal = Number(String(input.value || "").replace(",", "."));
    if(!Number.isFinite(goal) || goal < 0){
        showToast("Bitte ein gültiges Sparziel eingeben.", "error");
        return;
    }
    ...
    try{
        await writeActivityLog("Sparkonto", "Sparziel", "Sparziel", {
            goal,
            gesetzt_von: currentEmployee?.name || currentUser?.email || "Unbekannt"
        });
        ...
        showToast("Sparziel gespeichert.", "success");
    }catch(error){
        console.error("Sparziel konnte nicht gespeichert werden:", error);
        showToast("Sparziel konnte nicht gespeichert werden.", "error");
    }
}
...
async function saveDonationEntry(){
  ...
  v1Donations.unshift(data); await writeActivityLog('Einnahmen','Spende',data.id,{betrag:amount}); await notifyBookkeeping({route:'einzahlung',title:'Neue Clan-Spende',type:'Spende',amount,from:payload.mitarbeiter_name,purpose:payload.zweck,createdBy:payload.mitarbeiter_name,date:payload.created_at});
...
async function saveIncomeEntry(){
  ...
  v1Income.unshift(data); await writeActivityLog('Einnahmen',payload.typ,data.id,payload); await notifyBookkeeping({route:'einzahlung',title:'Neue Einnahme',type:payload.typ,amount,purpose,createdBy:payload.erstellt_von_name,note:payload.notiz,date:payload.created_at}); v1Status('incomeStatus','Einnahme gespeichert.'); renderIncome(); renderOverview();
}
async function savePayoutEntry(){
  ...
  const {data,error}=await buchhaltungDB.from('buchhaltung_auszahlungen').insert(payload).select().single(); if(error){v1Status('payoutStatus',error.message,false);return;} v1Payouts.unshift(data); await writeActivityLog('Auszahlungen',type,data.id,payload); await notifyBookkeeping({route:'auszahlung',title:'Neue Auszahlung',type,amount,to:payload.mitarbeiter_name,purpose:payload.zweck,createdBy:payload.erstellt_von_name,date:payload.created_at}); v1Status('payoutStatus','Auszahlung gespeichert.'); renderPayouts(); renderOverview();
}
...
async function saveLoan(){
  ...
  await writeActivityLog('Geldverleih','Darlehen genehmigt',data.id,payload); await notifyBookkeeping({route:'geldverleih',title:'Darlehen genehmigt',type:'Darlehen',amount,total,from:'Clan',to:payload.mitglied_name,purpose:'Genehmigt und ausgezahlt',createdBy:currentEmployee?.name,date:payload.genehmigt_am}); v1Status('loanStatus','Darlehen genehmigt, ausgezahlt und gespeichert.'); renderLoans(); renderOverview();
}
...
async function payLoanRate(loanId){
  ...
  Object.assign(loan,{getilgt:newPaid,repaid_amount:newPaid,offenbetrag:remaining,remaining_amount:remaining,next_due_date:next?.faellig_am||null,status}); await writeActivityLog('Geldverleih','Rate eingezahlt',loan.id,{betrag:amount,rate:rate.raten_nummer}); await notifyBookkeeping({route:'geldverleih',title:'Darlehensrate eingezahlt',type:'Darlehensrate',amount,from:loan.mitglied_name||v1EmployeeName(loan.mitglied_id),to:'Clan',purpose:'Rate über /Clan bestätigt',createdBy:currentEmployee?.name||currentUser.email,date:now,status}); v1Status('loanStatus',status==='Abgeschlossen'?'Darlehen vollständig zurückgezahlt.':'Rate gespeichert. Nächste Rate wird angezeigt.'); renderLoans(); renderOverview();
}
