...
(() => {
  ...
  async function boot(){
    ...
    // Update the existing top cards using the actual combined V1 figures.
    const income=v1IncomeRows().reduce((s,r)=>s+r.amount,0), expense=v1PayoutRows().reduce((s,r)=>s+r.amount,0), open=V1.loans.filter(l=>String(l.status).toLowerCase()==="aktiv").reduce((s,l)=>s+loanOpen(l),0), interest=V1.rates.filter(r=>String(r.status||"").toLowerCase()!=="bezahlt").reduce((s,r)=>s+Number(r.zinsen||0),0), fees=V1.warnings.filter(w=>String(w.status||"").toLowerCase()==="bezahlt").reduce((s,r)=>s+Number(r.gebuehr||0),0);
    if($("currentClanBalance"))$("currentClanBalance").textContent=money(income-expense);if($("totalRevenue"))$("totalRevenue").textContent=money(wealth());if($("totalDeposits"))$("totalDeposits").textContent=money(income);if($("totalWithdrawals"))$("totalWithdrawals").textContent=money(expense);if($("totalOpenAmounts"))$("totalOpenAmounts").textContent=money(open);if($("totalAssets"))$("totalAssets").textContent=money(wealth());
