...
(function(){
    ...
    window.alert = function(message){
        ...
        let box = document.getElementById("ehrenmarktBookkeepingToast");
        if(!box){
            box = document.createElement("div");
            box.id = "ehrenmarktBookkeepingToast";
            box.style.cssText = "position:fixed;right:18px;bottom:18px;z-index:99999;max-width:420px;padding:14px 18px;border:1px solid rgba(212,175,55,.55);border-radius:12px;background:rgba(10,10,12,.96);color:#fff;box-shadow:0 10px 35px rgba(0,0,0,.45);font:14px/1.4 system-ui,sans-serif;";
            document.body.appendChild(box);
        }
        box.textContent = text;
        clearTimeout(window.__ehrenmarktToastTimer);
        window.__ehrenmarktToastTimer = setTimeout(()=>box.remove(), 4500);
    };
})();
...
function getEmployeeRoles(){
    ...
    return [currentEmployee.rang,currentEmployee.role,currentEmployee.rolle,currentEmployee.rank]
        .filter(Boolean).map(v=>String(v).trim().toLowerCase());
}

function getCurrentRank(){
    return getEmployeeRoles()[0] || "";
}

function getCurrentRole(){
    return String(currentEmployee?.role || "").trim();
}

function isStadtleitung(){
    return getEmployeeRoles().includes("stadtleitung");
}

function isLeitung(){
    return getEmployeeRoles().some(role=>["leitung","stadtleitung","gründer","gruender","leiter"].includes(role));
}

function isMitarbeiter(){
    if(isLeitung()) return false;
    return getEmployeeRoles().includes("mitarbeiter");
}
...
function canCreateDeposit(){
    return isLeitung() || isMitarbeiter();
}
...
async function loadCurrentEmployee(){
    ...
    try{
        const {data,error}=await buchhaltungDB.from(TABLE_EMPLOYEES).select("*");
        if(error) throw error;
        const list=Array.isArray(data)?data:[];
        const uid=String(currentUser.id||"");
        const email=String(currentUser.email||"").toLowerCase();
        const meta=currentUser.user_metadata||{};
        const username=String(meta.username||meta.minecraft_name||"").toLowerCase();
        currentEmployee=list.find(e=>String(e.user_id||"")===uid)||
            list.find(e=>String(e.email||"").toLowerCase()===email)||
            list.find(e=>String(e.name||"").toLowerCase()===username)||null;
        return currentEmployee;
    }catch(error){
        console.error("Mitarbeiter:",error);
...
function sortBookkeepingRows(list){
    if(!Array.isArray(list)) return [];
    return list.sort((a,b)=>{
        const av=new Date(a?.datum||a?.date||a?.created_at||a?.erstellt_am||0).getTime()||0;
        const bv=new Date(b?.datum||b?.date||b?.created_at||b?.erstellt_am||0).getTime()||0;
        return bv-av;
    });
}
...
async function loadBookkeepingData(){
    ...
    */
    ...
    bookings=sortBookkeepingRows(bookings);
    savingsTransactions=sortBookkeepingRows(savingsTransactions);
    orderSettlements=sortBookkeepingRows(orderSettlements);
    orderWorkers=sortBookkeepingRows(orderWorkers);
    cashChecks=sortBookkeepingRows(cashChecks);
    activityLogs=sortBookkeepingRows(activityLogs);
...
window.buchhaltungDiagnose=function(){return {supabase:hasSupabase(),user:currentUser?.id||null,employee:currentEmployee||null,employees:employees.length,bookings:bookings.length,savings:savingsTransactions.length,orders:orderSettlements.length,workers:orderWorkers.length,cashChecks:cashChecks.length,logs:activityLogs.length};};
