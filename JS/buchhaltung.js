...
async function loadSavingsGoal(){
    ...
    // Das Sparziel wird im vorhandenen Buchhaltungs-Protokoll
    // im Feld details.goal gespeichert.
    savingsGoalValue = numberValue(
        details.goal ?? latest.neue_werte?.goal ?? 0
    );
}
...
async function saveSavingsGoal(){
    ...
    try{
        ...
        if(!log){
            throw new Error("Das Sparziel konnte nicht in Supabase gespeichert werden.");
        }

        renderSavings();
        renderOverview();
        showToast("Sparziel wurde dauerhaft gespeichert.", "success");
    }catch(error){
        console.error("Sparziel speichern:", error);
        showToast(
            "Sparziel konnte nicht gespeichert werden: " + (error?.message || "Unbekannter Fehler"),
            "error"
        );
    }
}
