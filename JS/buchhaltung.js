...
async function sendDiscordNotification(payload = {}){
    ...
    if(
        !hasSupabase() ||
        !buchhaltungDB?.functions?.invoke
    ){

        console.warn(
            "Discord-Benachrichtigung: Supabase Functions nicht verfügbar."
        );
    ...
    try{
        ...
        const {
            ...
            await buchhaltungDB.functions.invoke(
                "buchhaltung",
                {
                    body: payload
                }
...
function applySavingsEndpoints(){
    ...
    const from =
        getElement("savingsFrom");

    const to =
        getElement("savingsTo");
    ...
    /*
       Einzahlung:
       Mitarbeiter -> Sparkonto

       Auszahlung:
       Sparkonto -> Clan-Kasse
    */
    from.value =
        isWithdrawal
            ? "Sparkonto"
    ...
    */
    ...
    console.log(
        "Buchhaltungsdaten geladen:",
        {
            ...
            normalerClankassenstand:
                getNormalClanBalance(),
...
/*
   Der angezeigte Clanstand umfasst:
   normale Clankasse + Sparkonto.
*/
...
async function(){
    ...
    */
    ...
    if(
        ...
        showToast(
            "Die Auszahlung überschreitet den aktuellen Clankassenstand."
        );
    ...
    const progress =
        savingsGoalValue > 0
            ? Math.min(
                ...
                Math.max(
                    0,
                    (balance / savingsGoalValue) * 100
                )
            )
            : 0;

    const progressElement =
        getElement("savingsProgress");
...
function(
    ...
    const numberElement =
        getElement("savingsNumber");

    const dateElement =
        getElement("savingsDate");
    ...
    const warningText =
        document.querySelector(
            "#savingsModal .modal-warning span"
        );

    if(warningText){

        warningText.textContent =
            "Die Buchung gehört zum Sparkonto und wird im Gesamt-Clanstand mitgerechnet.";
...
async function(){
    ...
    const isWithdrawalForSavings =
        isWithdrawal;

    const savingsFrom =
        isWithdrawalForSavings
            ? "Sparkonto"
            : bookkeepingName;
    ...
    try{
        ...
        await sendDiscordNotification({
            ...
            title:
                isWithdrawal
                    ? "💸 Sparkonto-Auszahlung"
                    : "🏦 Neue Sparkonto-Einzahlung",
        ...
        await sendDiscordNotification({
            ...
            title:
                Math.abs(difference) >= 0.01
                    ? "⚠️ Kassenabgleich mit Abweichung"
                    : "⚖️ Kassenabgleich",
...
window.saveSavingsTransaction = saveSavingsEntry;
