...
(() => {
    ...
    function renderOverview() {
        ...
        text("totalAssets", money(totalRevenueAmount() + state.savings.filter(s=>String(s.art||s.typ||"").toLowerCase()==="einzahlung").reduce((sum,s)=>sum+Number(s.betrag||0),0)));
    }
    ...
    function renderEmployees() {
        ...
        if(body) body.innerHTML=list.length?list.map(e=>{
            ...
            const salaries=state.workers.filter(w=>String(w.name||w.arbeiter_name||"").toLowerCase()===name.toLowerCase()).reduce((s,w)=>s+Number(w.gehalt||0),0);
            const orders=state.workers.filter(w=>String(w.name||w.arbeiter_name||"").toLowerCase()===name.toLowerCase()).length;
    ...
    window.saveOrderSettlement=async function(){
        ...
        if(state.currentWorkers.length){
            const rows=state.currentWorkers.map(w=>({auftragsabrechnung_id:data.id,name:w.name,gehalt:Number(w.salary||0),notiz:w.note||null}));
