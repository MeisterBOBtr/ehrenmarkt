...
let currentWorkers = [];
let currentEmployeeName = "Manuell";
...
function getBookkeepingSupabase(){
...
async function getCurrentEmployeeName(){
    ...
    if(currentEmployeeName !== "Manuell"){
        return currentEmployeeName;
    }

    try{
        const client = getBookkeepingSupabase();
        if(!client?.auth?.getUser) return currentEmployeeName;

        const {data:userData} = await client.auth.getUser();
        const user = userData?.user || null;
        if(!user) return currentEmployeeName;
        ...
        try{
            ...
            console.warn("Mitarbeitername konnte nicht aus employees geladen werden:", error);
        }
        ...
        if(fallback) currentEmployeeName = fallback;

    }catch(error){
        console.warn("Aktueller Mitarbeiter konnte nicht geladen werden:", error);
    }
...
async function sendDiscordNotification(payload){
    ...
    if(!client?.functions?.invoke){
        console.warn("Discord-Benachrichtigung: Supabase Functions ist nicht verfügbar.");
        return false;
    }
    ...
    try{
        ...
        if(error){
            console.error("Discord-Benachrichtigung fehlgeschlagen:", error);
            return false;
        }

        if(data?.error){
            console.error("Discord-Benachrichtigung:", data.error, data.details || "");
            return false;
        }

        console.log("Discord-Benachrichtigung gesendet:", payload?.type || "Buchhaltung");
        return true;
...
async function saveBooking(){
    ...
    const amount = numberValue(getValue("bookingAmount"));
    ...
    const booking = {
        id: getValue("bookingNumber") || "BK-" + String(bookings.length + 1).padStart(4,"0"),
        type: getValue("bookingType"),
        amount,
        from: getValue("bookingFrom"),
        to: getValue("bookingTo"),
        purpose: getValue("bookingPurpose"),
        category: getValue("bookingCategory"),
        orderNumber: getValue("bookingOrderNumber"),
        paymentMethod: getValue("bookingPaymentMethod"),
        date: getValue("bookingDate") || nowLocal(),
        createdBy: employeeName,
        status: getValue("bookingStatus"),
        note: getValue("bookingNote")
    };
    ...
    activityLogs.push({
        ...
        type: "Erstellt",
        area: "Buchung",
        description: booking.id + " · " + booking.type + " · " + money(booking.amount),
    ...
    void sendDiscordNotification({
        type: "Buchung",
        title: booking.type === "Einzahlung" ? "💰 Neue Einzahlung" : "💸 Neue Auszahlung",
        ...
        status: booking.status || "Erstellt",
        createdBy: booking.createdBy,
        date: booking.date,
...
function saveOrderSettlement(){
    ...
    void sendDiscordNotification({
        ...
        from: "Auftrag",
        to: "Clan / Arbeiter",
        purpose: "Manuelle Auftragsabrechnung",
        category: "Auftrag",
        orderNumber: settlement.id,
        status: settlement.status || "Erstellt",
        createdBy: settlement.createdBy,
        date: settlement.date,
        clanAmount: settlement.clan,
        workerAmount: settlement.salaries,
        description: "Clananteil: " + money(settlement.clan) + " · Gehälter: " + money(settlement.salaries),
...
async function openSavings(){
    ...
    setValue("savingsNumber","SP-" + Date.now());
    setValue("savingsDate",nowLocal());

    const employeeName = await getCurrentEmployeeName();

    setValue("savingsFrom",employeeName);
    setValue("savingsTo","Sparkonto");

    const from = document.getElementById("savingsFrom");
    const to = document.getElementById("savingsTo");
...
async function saveSavings(){
    ...
    const type = getValue("savingsType");
    const amount = numberValue(getValue("savingsAmount"));

    if(!type || amount <= 0){
        alert("Bitte Art und einen gültigen Betrag eingeben.");
        return;
    }

    const employeeName = await getCurrentEmployeeName();
    const number = getValue("savingsNumber") || "SP-" + Date.now();
    const date = getValue("savingsDate") || nowLocal();
    const purpose = getValue("savingsPurpose");
    const note = getValue("savingsNote");

    /* Bei jeder Sparkonto-Buchung ist die Quelle der Mitarbeiter.
       Das Ziel ist fest das separate Sparkonto. */
    const from = employeeName;
    const to = "Sparkonto";

    setValue("savingsFrom",from);
    setValue("savingsTo",to);
    ...
    void sendDiscordNotification({
        type: "Sparkonto",
        title: type === "Einzahlung" ? "🏦 Neue Sparkonto-Einzahlung" : "🏦 Sparkonto-Auszahlung",
        amount,
        from,
        to,
        purpose,
        ...
        createdBy: employeeName,
        date,
        description: "Separates Clanvermögen / Spenden",
        note
    });
...
async function setSavingsGoal(){
    ...
    void sendDiscordNotification({
        ...
        category: "Sparkonto",
        status: "Gespeichert",
        createdBy: await getCurrentEmployeeName(),
        date: nowLocal(),
        description: "Sparziel: " + money(value)
    });
...
async function saveCashCheck(){
    ...
    const savedCheck = cashChecks[cashChecks.length - 1];

    void sendDiscordNotification({
        type: "Kassenabgleich",
        title: "⚖️ Neuer Kassenabgleich",
        amount: difference,
        from: "Portal / Clankasse",
        to: "Ingame-Kasse",
        purpose: "Kassenabgleich",
        category: "Kassenabgleich",
        status: difference === 0 ? "Übereinstimmend" : "Abweichung",
        createdBy: savedCheck?.createdBy || await getCurrentEmployeeName(),
        date: savedCheck?.date || nowLocal(),
        description: "Portal: " + money(portal) + " · Ingame: " + money(ingame),
        note: savedCheck?.note || ""
    });
...
function updateFinancialOverview(){
    ...
    const balanceDescription =
        document.querySelector(".balance-description");

    if(balanceDescription){
        balanceDescription.textContent =
            "Gesamt-Clanvermögen aus normaler Clankasse und Sparkonto.";
    }
...
function getManualClanBalance(){

    const income =
        bookings
            .filter(booking => booking.type === "Einzahlung")
            .reduce((sum, booking) => sum + numberValue(booking.amount), 0);

    const expenses =
        bookings
            .filter(booking => booking.type === "Auszahlung")
            .reduce((sum, booking) => sum + numberValue(booking.amount), 0);
...
function getCurrentClanBalance(){
    ...
    /* Gesamt-Clanstand = normale Clankasse + Sparkonto */
    return getManualClanBalance() + getSavingsBalance();
}
