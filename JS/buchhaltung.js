...
(() => {
    ...
    function renderOverview() {
        text("currentClanBalance", money(clanBalance() + savingsBalance()));
        ...
        text("totalAssets", money(bookingIncome() + state.savings.filter(s=>String(s.status||"").toLowerCase()!=="storniert" && String(s.art||s.typ||"").toLowerCase()==="einzahlung").reduce((sum,s)=>sum+Number(s.betrag||0),0)));
    }
    ...
    window.saveCashCheck=async function(){
        ...
        const payload={portalstand:portal,ingame_stand:ingame,abweichung:diff,notiz:value("cashCheckNote")||null,erstellt_von:state.user?.id||null,erstellt_von_name:employeeName(),created_at:new Date().toISOString()};
        ...
        state.cashChecks.unshift(data); await writeLog("Kassenabgleich","Erstellt","KA",payload); closeCashCheck(); renderAll();
    };
