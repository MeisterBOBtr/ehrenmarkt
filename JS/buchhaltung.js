...
/* =====================================================
   ...
   Die ursprüngliche Buchhaltung enthält die vollständige
   Anzeige-/Navigationslogik. Hier werden nur die vorhandenen
   Supabase-Daten in genau diese Arrays geladen.
===================================================== */

async function loadBookkeepingData(){
    if(!buchhaltungDB || typeof buchhaltungDB.from !== "function"){
        console.error("Buchhaltung: Supabase-Client nicht verfügbar.");
        return;
    }

    const read = async (table, orderField, ascending=false) => {
        const query = buchhaltungDB.from(table).select("*");
        const result = orderField
            ? await query.order(orderField, {ascending})
            : await query;
    ...
    const jobs = await Promise.allSettled([
        read("buchhaltung_buchungen", "datum", false),
        read("buchhaltung_auftragsabrechnungen", "datum", false),
        read("buchhaltung_auftragsarbeiter", "created_at", true),
        read("employees", "name", true),
        read("buchhaltung_sparkonto", "datum", false),
        read("buchhaltung_kassenabgleich", "datum", false),
        read("buchhaltung_protokoll", "datum", false)
    ]);
    ...
    jobs.forEach((result, index) => {
        if(result.status === "fulfilled"){
            targets[index].splice(0, targets[index].length, ...(result.value || []));
        }else{
            console.error("Buchhaltung – " + names[index] + " konnte nicht geladen werden:", result.reason);
            targets[index].splice(0, targets[index].length);
        }
    });
...
document.addEventListener(
    ...
    async () => {
        ...
        try {
            ...
            console.error("Buchhaltung: Daten konnten nicht vollständig geladen werden:", error);
        }
