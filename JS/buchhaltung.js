...
(() => {
    ...
    window.saveSavingsTransaction=async function(){
        ...
        const savingsFrom = type === "Auszahlung" ? "Sparkonto" : "Clan-Kasse";
        const savingsTo = type === "Auszahlung" ? employeeName() : "Sparkonto";
    ...
    function renderSavings(){
        ...
        body.innerHTML=state.savings.length?state.savings.map(s=>`<tr><td>${esc(s.buchungsnummer||"—")}</td><td>${esc(s.art||s.typ)}</td><td>${money(s.betrag)}</td><td>${esc(s.von || (String(s.art||s.typ||"").toLowerCase()==="auszahlung" ? "Sparkonto" : "Clan-Kasse"))}</td><td>${esc(s.an || (String(s.art||s.typ||"").toLowerCase()==="auszahlung" ? employeeName() : "Sparkonto"))}</td><td>${esc(s.zweck)}</td><td>${esc(dateText(s.datum||s.created_at))}</td><td>${esc(s.erstellt_von_name||s.erstellt_von)}</td><td>${esc(s.status||"Gespeichert")}</td></tr>`).join(""):`<tr class="empty-row"><td colspan="9">Noch keine Sparkonto-Buchungen vorhanden.</td></tr>`;
    }
