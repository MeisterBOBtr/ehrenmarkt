...
/* =========================================================
   EHRENMARKT TOASTS – KEINE BROWSER-ALERTS
========================================================= */
(function(){
    ...
    style.textContent = `
        ...
        .ehrenmarkt-bookkeeping-toast{
            pointer-events:auto;
            padding:14px 16px;
            border:1px solid rgba(212,175,55,.45);
            border-radius:12px;
            background:linear-gradient(135deg,rgba(8,8,8,.98),rgba(25,21,8,.96));
            color:#fff;
            box-shadow:0 14px 40px rgba(0,0,0,.55);
            font:14px/1.45 Arial,Helvetica,sans-serif;
            animation:ehrenmarktToastIn .2s ease;
        }
        .ehrenmarkt-bookkeeping-toast.success{border-color:rgba(86,200,120,.6)}
        .ehrenmarkt-bookkeeping-toast.error{border-color:rgba(217,92,92,.7)}
        .ehrenmarkt-bookkeeping-toast.warning{border-color:rgba(217,154,66,.7)}
        @keyframes ehrenmarktToastIn{
    ...
    window.showToast = function(message,type="info"){
        let root=document.getElementById("ehrenmarktBookkeepingToasts");
        if(!root){
            root=document.createElement("div");
            root.id="ehrenmarktBookkeepingToasts";
            document.body.appendChild(root);
        }
        const item=document.createElement("div");
        item.className="ehrenmarkt-bookkeeping-toast "+String(type||"info");
...
/* Der Ersteller wird ausschließlich aus dem eingeloggten
   Supabase-Benutzer + Mitarbeiterdatensatz bestimmt.
   Es gibt KEIN frei wählbares Buchhaltungsprofil mehr. */
...
function getSystemEmployeeName(){
    ...
    return String(name).trim() || "Unbekannt";
}


function ensureBookkeepingName(){
    return getSystemEmployeeName();
}

function getBookingEndpoints(type){
    const employeeName = getSystemEmployeeName();
    const normalized = String(type || "Einzahlung").trim().toLowerCase();
    if(normalized === "auszahlung") {
        return { von: "Clan-Kasse", an: employeeName };
    }
    return { von: employeeName, an: "Clan-Kasse" };
}

function applyBookingEndpoints(){
    const type = getValue("bookingType") || "Einzahlung";
    const endpoints = getBookingEndpoints(type);
    const from = getElement("bookingFrom");
    const to = getElement("bookingTo");
    ...
    if(from) from.value = endpoints.von;
    if(to) to.value = endpoints.an;
}

function bindBookingEndpointControls(){
    const type = getElement("bookingType");
    if(type && !type.dataset.endpointBound){
        type.addEventListener("change", applyBookingEndpoints);
        type.dataset.endpointBound = "true";
    }
...
function bindSavingsEndpointControls(){
    const type=getElement("savingsType");
    if(type && !type.dataset.endpointBound){
        type.addEventListener("change",applySavingsEndpoints);
        type.dataset.endpointBound="true";
    }
    applySavingsEndpoints();
}
...
/* =========================================================
   SPARKONTO – SYSTEMKONTROLLIERTE VON/AN
========================================================= */
function applySavingsEndpoints(){
    const type = getValue("savingsType") || "Einzahlung";
    const from = getElement("savingsFrom");
    const to = getElement("savingsTo");
    if(!from || !to) return;

    const isWithdrawal = String(type).toLowerCase() === "auszahlung";
    from.value = isWithdrawal ? "Sparkonto" : "Clan-Kasse";
    to.value = isWithdrawal ? "Clan-Kasse" : "Sparkonto";
...
async function loadSavingsGoal(){
    ...
    try{
        const key = currentUser?.id
            ? `ehrenmarkt_buchhaltung_sparziel_${currentUser.id}`
            : "ehrenmarkt_buchhaltung_sparziel";
        value = numberValue(localStorage.getItem(key));
    }catch(error){
        console.warn("Sparziel konnte nicht geladen werden:", error);
    }
    savingsGoalValue = Math.max(0, value);
    const input = getElement("savingsGoal");
    if(input) input.value = savingsGoalValue || "";
...
function updateSavingsGoalProgress(){
    ...
    const goal = Math.max(0, numberValue(savingsGoalValue));
    const bar = getElement("savingsProgress");
    const text = getElement("savingsProgressText");
    const display = getElement("savingsGoalDisplay");

    if(display) display.textContent = money(goal);

    if(goal <= 0){
        if(bar) bar.style.width = "0%";
        if(text) text.textContent = "0 % erreicht";
        return;
    }

    const percent = Math.min(100, Math.max(0, (balance / goal) * 100));
    if(bar) bar.style.width = percent + "%";
    if(text) text.textContent = percent.toFixed(1) + " % erreicht";
}
...
window.addOrderWorker = function(){
    ...
    if(!canManageBookkeeping()){
        showToast("Nur Leitung und Stadtleitung dürfen Arbeiter hinzufügen.", "error");
        return;
    }

    let modal = document.getElementById("buchhaltungWorkerModal");
    if(!modal){
        modal = document.createElement("div");
        modal.id = "buchhaltungWorkerModal";
        modal.className = "modal active";
        modal.innerHTML = `
            <div class="modal-content" style="max-width:520px;">
                <div class="modal-header">
                    <h3>Arbeiter hinzufügen</h3>
                    <button type="button" class="modal-close" data-worker-close>×</button>
                </div>
                <div style="display:grid;gap:12px;">
                    <label>Arbeiter
                        <select id="buchhaltungWorkerSelect">
                            <option value="">Arbeiter auswählen</option>
                        </select>
                    </label>
                    <label>Gehalt ($)
                        <input id="buchhaltungWorkerSalary" type="number" min="0" step="1" placeholder="0">
                    </label>
                    <label>Notiz (optional)
                        <textarea id="buchhaltungWorkerNote" rows="3" placeholder="Optional"></textarea>
                    </label>
                    <div style="display:flex;gap:10px;justify-content:flex-end;">
                        <button type="button" class="secondary-button" data-worker-close>Abbrechen</button>
                        <button type="button" class="primary-button" id="buchhaltungWorkerSave">Arbeiter hinzufügen</button>
                    </div>
                </div>
            </div>`;
    ...
    const select = modal.querySelector("#buchhaltungWorkerSelect");
    const salary = modal.querySelector("#buchhaltungWorkerSalary");
    const note = modal.querySelector("#buchhaltungWorkerNote");
    select.innerHTML = '<option value="">Arbeiter auswählen</option>' +
        (Array.isArray(employees) ? employees.map(e => {
            const id = e.id || e.user_id || e.minecraft_name || e.username;
            const name = e.minecraft_name || e.name || e.username || e.display_name || e.email || "Unbekannt";
    ...
    modal.querySelectorAll("[data-worker-close]").forEach(b => b.onclick = close);
    modal.querySelector("#buchhaltungWorkerSave").onclick = () => {
        const id = select.value;
        const employee = Array.isArray(employees) ? employees.find(e => String(e.id || e.user_id || e.minecraft_name || e.username) === String(id)) : null;
        const cleanName = employee ? (employee.minecraft_name || employee.name || employee.username || employee.display_name || employee.email) : "";
        const value = numberValue(salary.value);
        if(!cleanName){ showToast("Bitte einen Arbeiter auswählen.", "error"); return; }
        if(value < 0){ showToast("Das Gehalt darf nicht negativ sein.", "error"); return; }
        currentWorkers.push({ name: String(cleanName).trim(), salary: value, note: String(note.value || "").trim(), employee_id: employee?.id || employee?.user_id || null });
        renderCurrentWorkers();
        updateWorkerTotals();
        close();
    };
...
async function(){
    ...
    const bookkeepingName = getSystemEmployeeName();
const payload = {
...
window.saveSavingsGoal = function(){
    if(!canManageBookkeeping()){
        showToast("Keine Berechtigung zum Ändern des Sparziels.","error");
        return;
    }
    ...
    const value = Math.max(0, numberValue(input.value));
    savingsGoalValue = value;
    input.value = value;

    // Das HTML besitzt für das Sparziel keine eigene Datenbankspalte.
    // Deshalb wird das Ziel pro eingeloggtem Benutzer dauerhaft im Browser gespeichert.
    try{
        const key = currentUser?.id
            ? `ehrenmarkt_buchhaltung_sparziel_${currentUser.id}`
            : "ehrenmarkt_buchhaltung_sparziel";
        ...
        console.warn("Sparziel konnte lokal nicht gespeichert werden:", error);
    }

    updateSavingsGoalProgress();
    renderSavings();
    renderOverview();
    showToast("Sparziel wurde gespeichert.","success");
};
...
async function initializeBookkeeping(){
    ...
    try{
        ...
        /*
           ...
           Der Ersteller wird ausschließlich
           aus dem eingeloggten Benutzer
           und dessen Mitarbeiterprofil bestimmt.
        */

        await loadCurrentEmployee();
        bindBookingEndpointControls();
        bindSavingsEndpointControls();
...
/* ================================
   DOM START / INITIALISIERUNG
================================ */

if(document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", initializeBookkeeping, {once:true});
}else{
...
window.saveOrderSettlement = saveOrderSettlement;
window.addOrderWorker = addOrderWorker;
window.removeOrderWorker = removeOrderWorker;
window.filterOrderSettlements = filterOrderSettlements;
window.deleteOrderSettlement = deleteOrderSettlement;
...
window.saveSavingsEntry = saveSavingsEntry;
window.saveSavingsTransaction = window.saveSavingsTransaction;
window.openCashCheckModal = openCashCheckModal;
window.closeCashCheckModal = closeCashCheckModal;
window.saveCashCheck = saveCashCheck;
...
window.renderCashChecks = renderCashChecks;
window.renderMonthlyOverview = renderMonthlyOverview;
...
========================================================= */
/* =========================================================
   ARBEITER-MODAL – EINHEITLICHE FINANCE-MODAL-VERSION
========================================================= */
window.openWorkerModal = function(){
    ...
    const modal=getElement("workerModal");
    if(!modal) return;

    const name=getElement("workerName");
    const salary=getElement("workerSalary");
    if(name){
        name.innerHTML='<option value="">Mitarbeiter auswählen</option>' +
            (Array.isArray(employees) ? employees.map(e=>{
                const id=e.id || e.user_id || e.minecraft_name || e.username;
                const label=e.minecraft_name || e.name || e.username || e.display_name || e.email || "Unbekannt";
                return `<option value="${escapeHtml(String(label))}">${escapeHtml(String(label))}</option>`;
            }).join("") : "");
    }
    const note=getElement("workerNote");
    if(name) name.value="";
...
window.saveWorker = function(){
    ...
    const name=String(getValue("workerName")||"").trim();
    const salary=numberValue(getValue("workerSalary"));
    const note=String(getValue("workerNote")||"").trim();

    if(!name){
        showToast("Bitte einen Arbeiter auswählen bzw. den Minecraft-Namen eingeben.","error");
        return;
    }
    if(salary<0){
        showToast("Das Gehalt darf nicht negativ sein.","error");
        return;
    }

    const employee = Array.isArray(employees)
        ? employees.find(e=>String(
            e.minecraft_name || e.name || e.username || e.display_name || e.email || ""
        ).trim() === name)
        : null;
    ...
    currentWorkers.push({
        ...
        salary,
        note,
        employee_id: employee?.id || employee?.user_id || null
    });
    renderCurrentWorkers();
    updateWorkerTotals();
    closeWorkerModal();
    showToast("Arbeiter wurde zur Auftragsabrechnung hinzugefügt.","success");
};

/* HTML-Kompatibilitätsnamen */
window.addWorkerRow = window.openWorkerModal;


/* =========================================================
   HTML-HANDLER – KOMPATIBILITÄTS-ALIASE
========================================================= */
window.filterOrders = function(){ 
    if(typeof filterOrderSettlements === "function") return filterOrderSettlements();
};
window.filterLogs = function(){
    if(typeof filterActivityLogs === "function") return filterActivityLogs();
};
window.openCashCheck = function(){
    if(typeof openCashCheckModal === "function") return openCashCheckModal();
};
window.closeCashCheck = function(){
    if(typeof closeCashCheckModal === "function") return closeCashCheckModal();
};
window.closeLogDetail = function(){
    if(typeof closeActivityLogDetail === "function") return closeActivityLogDetail();
    const modal=getElement("logDetailModal");
    if(modal) modal.classList.remove("active");
};
window.saveSavingsTransaction = function(){
    if(typeof window.saveSavingsEntry === "function") return window.saveSavingsEntry();
};
window.updateMonthlyBalance = function(){
    if(typeof renderMonthlyOverview === "function") return renderMonthlyOverview();
};
